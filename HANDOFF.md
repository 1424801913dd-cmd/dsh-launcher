...
## 二十七、0.4.1 版本漂移修复候选（2026-09-04）

### 缺陷与处理边界

- 测试端 Windows 10 Home China 22H2 实体机回传 P1：0.4.0 向导显示确认 `0.1.1-rc.2`，实际装入 `0.1.2-rc.1`。旧候选停止作为交接推荐包，保留 `trial-step4-handoff-v2` 及报告，不删除或改写错误现场。
- 初始快照的硬编码版本和仅传通道的安装命令未绑定用户确认值；不能依据事后 npm 页面认定当时 registry/CDN 出错。
- 新版移除未查询版本、传递不可变 `expectedVersion`、核对后台成功查询快照、再次解析标签但只作一致性检查、请求精确 manifest，并在安装验证及首次写活动指针前核对版本。漂移/失败要求重新查询确认，推荐版和 Alpha 同样受保护。
- 卸载入口缺失单独跟踪：NSIS 源码包含当前用户注册表写入；新增四视图只读检查、静默/Passive 两模式安装后及启动后注册验证、卸载后移除验证，不手工补注册表掩盖问题。Windows 10 应用列表的实际表现仍待测试端回传。

### 本地与 CI 验证

- 本地前端 4 项、Rust 34 项、卸载注册合成检查 8 项通过；0.4.1 Release 构建及隔离启动通过。生产 `active.json` 哈希前后为 `98032014819D333CD8E84AA4319622F595A9E1529B7648B0ADCAC74F94F7CE43`。
- 本地推荐版从零安装、隔离启停和精确版本断言通过（本次目标 `0.1.2-rc.1`）；收尾 PowerShell 进程查询遇沙箱 CIM 拒绝，随后只读提升查询确认残留受管进程为 0，生产指针不变。原测试目录保留，补充记录为 `phase4-results/clean-runtime-0.4.1-local-supplement.json`，不冒充未经中断的自动报告。
- 首轮 CI `33856698398` 揭示测试 HTTP 服务器接受的 Windows socket 继承非阻塞模式；已明确设为阻塞并保留读写超时，本地版本选择套件连续 10 轮通过。`33857307493` 在同一问题尚未修复的提交上启动，已取消且确认终态。
- 最终候选受测提交 `9782067f3f358a7c5458eddb8464afb66acead4f`，CI `33857469193` 全部通过，Windows Server 2025，17 分 37 秒；前端 4、Rust 34、卸载注册规则 8 项通过，推荐版从零安装/隔离启停通过，各处版本均为 `0.1.2-rc.1`。静默/Passive 两模式安装、首次启动后的卸载注册和卸载清除、数据保留均通过。未合并 main、未创建 Release、未修改远程更新或签名门禁。

### 新交接要求

- ZIP 增加 `VERSION-DRIFT-RETEST.md`、只读卸载检查及两安装模式报告。测试端先读 `CODEX-HANDOFF.md`，保留错误 Runtime/日志/更早归档，经用户批准后精确隔离再复测，不能默认新包会自动清理旧状态。
- 桌面记录器 schema v3 要求 `VersionConsistency` 和 `UninstallEntry` 实测观察；缺失项保持 NOT_RUN。推荐/Alpha 的模拟版本漂移测试不等于 Alpha 全量公网安装或人工桌面通过。
